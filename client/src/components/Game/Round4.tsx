...
const Round4 = () => {
  ...
  const [word,setWord] = useState<wordType>({
    ...
    hint : "Electric appliance serves the purpose of cooling",
    clue1 : "important component : CFC (Chlorofluorocarbon)",
    clue2 : "ABBREVIATION : AC",
    displayWord : "A_R C___IT_O_E_"
})
...
const handleExit = ()=>{
  ...
  return loading?(
    <Center minH="80vh">
        <Heading color="white" size="xl" fontFamily="Silkscreen, cursive">Final Round Starts in {count}</Heading>
    </Center>
  ):(
    <Flex justifyContent="start" w="full" minH="80vh" bg="blackAlpha.700" direction={{base : 'column', sm : 'column', md : 'row'}}>
      ...
      <Flex direction={{base : 'row', sm: 'row', md : "column" }} justifyContent="center" alignItems="center" flex={2} gap={6}>
        <Heading size="md" color="white">Players</Heading>
        {
          players.map((el,i)=>{
            return (
      ...
      <VStack w="full" border="1px solid white" p={2} flex={8}>
        <VStack w="full" border="2px solid white" p={2} h="full" justifyContent={'space-between'}>
          <HStack w="full" justifyContent="space-between">
            ...
            <Text fontSize="2xl" fontFamily="Silkscreen, cursive" color="white">FINAL ROUND</Text>
          ...
          <VStack>
            {
              submit?(
                ...
                <HStack gap={2}>
                  ...
                  <Text color="blue.500">Wait till game ends!</Text>
                </HStack>
                </>
              ):(
                <>
